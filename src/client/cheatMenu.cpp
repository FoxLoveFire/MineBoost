...
Menu::Menu(Client *client) : m_client(client)
{
	...
	bg_color = g_settings->getV3F("menu_bg_color");
	active_bg_color = g_settings->getV3F("menu_active_bg_color");
	font_color = g_settings->getV3F("menu_font_color");
	selected_font_color = g_settings->getV3F("menu_selected_font_color");
	...
	if (!m_font) {
		errorstream << "OptifineMenu: Unable to load font" << std::endl;
	} else {
...
void Menu::draw(video::IVideoDriver *driver, bool show_debug)
{
	...
	if (!show_debug)
		drawEntry(driver, "MineBoost", 0, false, false,
